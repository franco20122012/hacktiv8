...
class Employee {
  ...
  static registerEmployee (username, password, role, cb) {
    ...
    this.read((err, data) => {
      if (err) {
        return cb(err)
      } else { // Check if the username is already taken
        for (let i = 0; i < data.length; i++) {
        ...
        // If username is valid, push the new employee to the array
  ...
  static update (updateObj, cb) {
    this.read((err, employees) => {
      if (err) {
        return cb(err)
      } else { // Find the object employee that you want to update
        for (let i = 0; i < employees.length; i++) {
  ...
  static checkLogValidity (cb) {
    this.read((err, employees) => {
      ...
      // Return error message if no user is logged in
      if (!found) {
